...
import {
  getAccounts, createAccount, deleteAccount, addAccountRole, updateAccountRole, removeAccountRole,
  type AccountSummary,
...
function AccountEditPanel({
  ...
  async function handleSaveRole(chapterId: number) {
    ...
    try {
      ...
      onClose(); onDeleted() // reload
    } catch (e) {
      setRowError(prev => ({ ...prev, [chapterId]: e instanceof Error ? e.message : 'Failed to save' }))
    } finally {
  ...
  async function handleRemove(chapterId: number) {
    ...
    try {
      ...
      setRowError(prev => ({ ...prev, [chapterId]: e instanceof Error ? e.message : 'Failed to remove' }))
  ...
  return (
    <div className="border-t border-slate-100 bg-slate-50 px-6 py-5">
      ...
      <form onSubmit={handleAdd} className="flex items-end gap-3 mb-5">
        ...
        <button type="submit" disabled={!addChapter || !addRole || addLoading}
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-4">New Account</p>
              ...
              {accounts.map(account => (
                <>
                  <tr
                    ...
                    onClick={() => (isAdmin || isChapterAdmin) && setExpandedId(id => id === account.id ? null : account.id)}
                  >
                    ...
                    <td className="px-6 py-3">
                      <div className="flex flex-wrap gap-2">
                        {account.chapterRoles.map(cr => (
                          <span key={cr.chapterId} className="inline-flex items-center gap-1 text-xs text-slate-500">
                    ...
                    <tr key={`${account.id}-edit`}>
                      <td colSpan={4} className="p-0">
                        <AccountEditPanel
                          ...
                          onDeleted={() => { setExpandedId(null); loadAccounts() }}
                        />
                      </td>
                    </tr>
                  )}
