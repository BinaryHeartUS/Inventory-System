...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function ManageIndividuals() {
  const { auth } = useAuth()
  const [individuals, setIndividuals] = useState<PartySummary[]>([])
  ...
  return (
    <div>
      <PageHeading title="Manage Individuals" subtitle="Create and edit individual donors and recipients." />
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-slate-400">
            <p className="text-sm">{individuals.length === 0 ? 'No individuals yet.' : 'No matches.'}</p>
          </div>
        ) : (
