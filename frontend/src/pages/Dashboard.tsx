...
export default function Dashboard() {
  ...
  const selectedChapterIds: number[] = selectedChapter === 'All'
  ...
  const completionPct = completionRate && completionRate.total > 0
    ? Math.round((completionRate.donated / completionRate.total) * 100)
    : 0
  const avgDays    = avgTime?.avgDays !== null ? (avgTime?.avgDays != null ? Math.round(avgTime.avgDays) : null) : null
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Not Started',     count: notStartedCount,    bg: 'bg-slate-100', text: 'text-slate-700', dot: 'bg-slate-400' },
            { label: 'In Progress',     count: inProgressCount,    bg: 'bg-amber-50',  text: 'text-amber-700', dot: 'bg-amber-400' },
            { label: 'Ready to Donate', count: readyToDonateCount, bg: 'bg-green-50',  text: 'text-green-700', dot: 'bg-green-500' },
            { label: 'Donated',         count: donatedCount,       bg: 'bg-sky-50',    text: 'text-sky-700',   dot: 'bg-sky-500'   },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col">
          <div className="flex items-baseline justify-between mb-4">
            ...
            <span className="text-lg font-extrabold text-heart-blue">{totalCount !== null ? totalCount : '—'} total</span>
          </div>
              ...
              <div key={label}>
                <div className="flex justify-between text-xs mb-1.5">
                  ...
                  <span className="text-slate-700 font-semibold">
                    ...
                    <span className="text-slate-400 font-normal">
                      {count !== null && totalCount ? `${Math.round((count / totalCount) * 100)}%` : '—'}
                    </span>
                  </span>
                </div>
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: count !== null && totalCount ? `${(count / totalCount) * 100}%` : '0%' }}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between">
          ...
          <p className="text-[11px] text-slate-300 mt-4">
            Based on {sampleSize} donated device{sampleSize !== 1 ? 's' : ''} with both dates recorded
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center py-3">
            <div className="flex items-end gap-2">
              <span className="text-3xl font-extrabold text-heart-blue leading-none">{completionPct}%</span>
              <span className="text-xs text-slate-400 mb-0.5">
                {completionRate ? `${completionRate.donated} of ${completionRate.total} devices` : '—'}
              </span>
            </div>
            <div className="mt-2 h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-heart-blue rounded-full transition-all" style={{ width: `${completionPct}%` }} />
            </div>
          </div>
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-semibold text-slate-800">
                  {chapterActivity ? `${chapterActivity.activeChapters} of ${chapterActivity.totalChapters}` : '—'}
                </span>
              </div>
              <div className="flex justify-between text-xs">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className={`font-semibold ${(chapterActivity?.chaptersWithPickupsReady ?? 0) > 0 ? 'text-green-600' : 'text-slate-300'}`}>
      ...
      {/* Activity over time */}
      <ActivityChart receivedData={receivedData} donatedData={donatedActivityData} loading={chartsLoading} />
