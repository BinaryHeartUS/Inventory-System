...
export interface AdminAccountsViewProps {
  ...
  /** Creates the account; resolves with an error message, or null on success. */
  onCreate: (payload: CreateAccountPayload) => Promise<string | null>;
  onReload: () => void;
}

/**
 * AdminAccountsView — presentational account management: the new-account form
 * (owns its own draft state) and the chapter-grouped account list with inline
 * edit panels. Data + create/reload actions come from props.
 */
