...
export function PartyPickerModal({
  ...
  const filtered = useMemo(() => {
    ...
    return parties.filter(
      (p) => p.name.toLowerCase().includes(s) || (p.type ?? "").toLowerCase().includes(s)
    );
  }, [parties, search]);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b border-slate-100">
                    <th className="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 font-normal">
                      Name
                    </th>
                    <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 font-normal">
                      Type
                    </th>
                  </tr>
                </thead>
