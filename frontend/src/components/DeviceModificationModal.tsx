...
function buildFields(entry: DeviceChangelogEntry): FieldDef[] {
  return [
    { label: 'Manufacturer',      old: fmtText(entry.oldManufacturer),     new: fmtText(entry.newManufacturer) },
    ...
    { label: 'Donated Date',      old: fmtDate(entry.oldDonatedDate),      new: fmtDate(entry.newDonatedDate) },
    ...
    { label: 'RAM Generation',    old: fmtText(entry.oldRamGeneration),    new: fmtText(entry.newRamGeneration) },
    { label: 'Storage (GB)',      old: fmtNum(entry.oldStorageAmount),     new: fmtNum(entry.newStorageAmount) },
    { label: 'Storage Type',      old: fmtText(entry.oldStorageType),      new: fmtText(entry.newStorageType) },
    { label: 'Operating System',  old: fmtText(entry.oldOperatingSystem),  new: fmtText(entry.newOperatingSystem) },
    { label: 'Has Wi-Fi',         old: fmtBool(entry.oldHasWifi),          new: fmtBool(entry.newHasWifi) },
    { label: 'Includes Charger',  old: fmtText(entry.oldIncludesCharger),  new: fmtText(entry.newIncludesCharger) },
    { label: 'Design Capacity',   old: fmtNum(entry.oldDesignCapacity),    new: fmtNum(entry.newDesignCapacity) },
    { label: 'Actual Capacity',   old: fmtNum(entry.oldActualCapacity),    new: fmtNum(entry.newActualCapacity) },
    { label: 'Battery Health',    old: fmtPct(entry.oldBatteryHealth),     new: fmtPct(entry.newBatteryHealth) },
    { label: 'Working Battery',   old: fmtText(entry.oldWorkingBattery),   new: fmtText(entry.newWorkingBattery) },
  ]
}
