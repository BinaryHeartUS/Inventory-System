...
export default function PartyDetailView({
  ...
  const allDonatedById = new Map<number, ReceiptItem>([
    ...
    ...donatedMisc.map(
      (item) =>
        [item.id, { id: item.id, label: item.description, year: null, value: item.value }] as [
          number,
          ReceiptItem,
        ]
    ),
        ...
        <div className="space-y-3">
          ...
          {donatedOpen && (
            <div className="space-y-3">
              ...
              <SectionCard title="Donated Misc" count={donatedMisc.length} icon={miscIcon}>
                <DonatedTable
                  rows={donatedMisc.map((item) => ({
                    id: item.id,
                  ...
                  basePath="/misc"
                  emptyMessage="No donated misc assets."
                  selectMode={selectMode}
                  selected={selected}
                  onToggle={toggle}
                  onToggleAll={toggleAll}
