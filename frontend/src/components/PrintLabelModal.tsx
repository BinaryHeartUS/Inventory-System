...
export function PrintLabelModal({ assetId, onClose }: {
  assetId: number
  onClose: () => void
}) {
  ...
  useEffect(() => {
    if (canvasRef.current) { drawLabel(canvasRef.current, assetId) }
  }, [assetId])
  ...
  async function handlePrint() {
    ...
    try {
      ...
      try { client.disconnect() } catch { /* ignore disconnect errors */ }
    }
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-start justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Asset{' '}
              <span className="font-mono font-medium text-slate-600">#{paddedId}</span>
              {' '}added successfully.
            </p>
          </div>
          <button
        ...
        {!serialSupported && (
          <div className="mx-6 mt-4 flex items-start gap-2.5 rounded-lg bg-amber-50 border border-amber-200 px-3.5 py-3">
            <svg className="shrink-0 mt-0.5 text-amber-500" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
              <line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
            </svg>
            <p className="text-xs text-amber-800 leading-snug">
              Printing requires a <span className="font-semibold">Chromium-based browser</span> (Chrome, Edge, Opera, etc.).
              Your current browser does not support the Web Serial API needed to communicate with the printer.
            </p>
          </div>
        )}
