...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        IF OLD.Includes_Charger IS DISTINCT FROM NEW.Includes_Charger OR
            OLD.Working_Battery IS DISTINCT FROM NEW.Working_Battery THEN
            INSERT INTO Tablet_Change_Log(Tablet_ID, Modified_By, Change_Type,
            ...
            VALUES (NEW.ID, COALESCE(current_setting('app.current_username', true), current_user),
                ...
                CASE WHEN OLD.Working_Battery IS DISTINCT FROM NEW.Working_Battery THEN OLD.Working_Battery END,
