CREATE TABLE IF NOT EXISTS Laptop (
    ...
    Battery_health NUMERIC GENERATED ALWAYS AS (
        CASE
            WHEN Design_battery_capacity IS NOT NULL AND Actual_battery_capacity IS NOT NULL AND Design_battery_capacity <> 0 THEN (Actual_battery_capacity::NUMERIC / Design_battery_capacity)
            ELSE NULL
        END
    ) STORED,
