...
public class SheetReader<T extends Record> {
  ...
  public static <T extends Record> T readRow(Sheet sheet, Class<T> recordClass, int rowIndex) {
    ...
    for (int i = 0; i < components.length; i++) {
      ...
      if (i == 0
          && (values[i] == null || (values[i] instanceof String str && str.strip().equals("")))) {
        return null;
      }
    }
  ...
  private static Object getCorrectFormatFor(Cell cell, Class<?> type) {
    ...
    if (type == int.class || type == Integer.class) return (int) cell.getNumericCellValue();
    if (type == double.class || type == Double.class) return cell.getNumericCellValue();
    if (type == boolean.class || type == Boolean.class) return cell.getBooleanCellValue();
    ...
    if (type.isEnum()) {
      ...
      if (type == OperatingSystem.class) return OperatingSystem.valueOf(cellValue);
      if (type == Status.class) return Status.valueOf(cellValue);
      if (type == TypeOfDevice.class) return TypeOfDevice.valueOf(cellValue);
      if (type == ChargerStatus.class) return ChargerStatus.valueOf(cellValue);
      if (type == WorkingBattery.class) return WorkingBattery.valueOf(cellValue);
    }
