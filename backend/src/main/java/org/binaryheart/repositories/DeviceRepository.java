	...
	public DeviceChangelogResponse[] getDeviceChangelog(Integer deviceID) throws SQLException {
		...
		while (rs.next()) {
			...
			entries.add(new DeviceChangelogResponse(deviceType, ID, modifiedBy, modifiedAt, changeType,
				...
				oldRamGeneration, newRamGeneration, oldStorageAmount, newStorageAmount, oldStorageType, newStorageType,
				oldStatus, newStatus, oldHasWifi, newHasWifi, oldIncludesCharger, newIncludesCharger, oldDesignCap,
				newDesignCap, oldActualCap, newActualCap, oldBatteryHealth, newBatteryHealth, oldWorkingBattery,
				newWorkingBattery, oldDonatedDate, newDonatedDate, oldOS, newOS, oldRecipientID, newRecipientID));
		}
