	...
	public List<GetDeviceResponse> getAllDevices() throws SQLException {
		try (Connection conn = DatabaseConnectionService.getConnection();
			...
			while (rs.next()) {
				...
				Integer donorId = rs.getObject("donor_id", Integer.class);
				Integer recipientId = rs.getObject("recipient_id", Integer.class);
				devices.add(new GetDeviceResponse(deviceType, deviceID, acquisitionLocalDate, value, manufacturer,
					model, year, cpu, ram, ramGeneration, storage, storageType, status, hasWifi, hasCharger, designCap,
					actualCap, batteryHealth, workingBattery, chapter, dateDonated, operatingSystem, donorId,
					recipientId));
			}
			return devices;
