...
public class DeviceRepository {
  ...
  public int getDeviceCountByChapters(String type, String status, List<Integer> chapterIds)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
    try (CallableStatement stmt =
        conn.prepareCall("call Get_Device_Count_By_Chapters(?, ?, ?, ?)")) {
  ...
  public DashboardCountsResponse getDashboardCounts(List<Integer> chapterIds) throws SQLException {
    ...
    Connection conn = DatabaseConnectionService.getConnection();
    try (CallableStatement stmt =
        conn.prepareCall("call Get_Dashboard_Counts(?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
  ...
  public AvgTimeInInventoryResponse getAvgTimeInInventory(List<Integer> chapterIds)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
  ...
  public ChapterActivityStatsResponse getChapterActivityStats(List<Integer> chapterIds)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
    try (CallableStatement stmt =
        conn.prepareCall("call Get_Chapter_Activity_Stats(?, ?, ?, ?, ?)")) {
      setIntegerArray(stmt, 1, chapterIds, conn);
      stmt.registerOutParameter(2, Types.INTEGER);
      stmt.registerOutParameter(3, Types.INTEGER);
      stmt.registerOutParameter(4, Types.INTEGER);
      stmt.registerOutParameter(5, Types.INTEGER);
      stmt.execute();
      return new ChapterActivityStatsResponse(
          stmt.getInt(2), stmt.getInt(3), stmt.getInt(4), stmt.getInt(5));
    }
  }

  public List<MonthlyCountPoint> getDevicesReceived(List<Integer> chapterIds, int months)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT yr, mo, count FROM Get_Devices_Received(?, ?)")) {
  ...
  public List<MonthlyCountPoint> getDevicesDonated(List<Integer> chapterIds, int months)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT yr, mo, count FROM Get_Devices_Donated(?, ?)")) {
      stmt.setArray(1, toSqlArray(chapterIds, conn));
      stmt.setInt(2, months);
      ResultSet rs = stmt.executeQuery();
      List<MonthlyCountPoint> points = new ArrayList<>();
  ...
  public List<MonthlyValuePoint> getDonatedDeviceValue(List<Integer> chapterIds, int months)
      throws SQLException {
    ensureConnected();
    Connection conn = DatabaseConnectionService.getConnection();
    Array arr = toSqlArray(chapterIds, conn);
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT yr, mo, total_value FROM Get_Donated_Device_Value(?, ?)")) {
      stmt.setArray(1, arr);
      stmt.setInt(2, months);
      ResultSet rs = stmt.executeQuery();
      ...
      while (rs.next()) {
        points.add(
            new MonthlyValuePoint(rs.getInt("yr"), rs.getInt("mo"), rs.getDouble("total_value")));
      }
      return points;
    }
  }
  ...
  private static void setIntegerArray(
      CallableStatement stmt, int paramIndex, List<Integer> ids, Connection conn)
      throws SQLException {
  ...
  public GetDeviceResponse getDevice(int id) throws SQLException, DeviceNotFoundException {
    ...
    LocalDate dateDonated =
        rs.getDate("Donated_Date") != null ? rs.getDate("Donated_Date").toLocalDate() : null;
  ...
  public void insertDesktop(InsertDesktopRequest request, String username) throws SQLException {
    ...
    try {
      PreparedStatement ps =
          conn.prepareStatement("SELECT set_config('app.current_username', ?, true)");
  ...
  public DeviceChangelogResponse[] getDeviceChangelog(Integer deviceID) throws SQLException {
    ...
    while (rs.next()) {
      ...
      if (rs.wasNull()) oldStorageAmount = null;
      Integer newStorageAmount = rs.getInt("New_Storage_Amount");
      if (rs.wasNull()) newStorageAmount = null;
      ...
      if (rs.wasNull()) oldHasWifi = null;
      Boolean newHasWifi = rs.getBoolean("New_HasWifi");
      if (rs.wasNull()) newHasWifi = null;
      ...
      if (rs.wasNull()) oldRecipientID = null;
      Integer newRecipientID = rs.getInt("New_Recipient_ID");
      if (rs.wasNull()) newRecipientID = null;
      entries.add(
          new DeviceChangelogResponse(
              deviceType,
              ID,
              modifiedBy,
              modifiedAt,
