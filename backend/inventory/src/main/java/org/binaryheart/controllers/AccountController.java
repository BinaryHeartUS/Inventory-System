...
public class AccountController {
  ...
  @OpenApi(
      ...
      description =
          "Admins can create Chapter Admin / Editor / Viewer accounts for any chapter. "
              + "Chapter Admins can create Editor / Viewer accounts for their own chapters.",
      requestBody =
          @OpenApiRequestBody(
              required = true,
              content = {@OpenApiContent(from = CreateAccountRequest.class)}),
      responses = {
        @OpenApiResponse(
            status = "201",
            description = "Account created",
            content = {@OpenApiContent(from = AccountSummary.class)}),
        @OpenApiResponse(status = "400", description = "Invalid request or forbidden role"),
        @OpenApiResponse(status = "409", description = "Username already taken"),
  ...
  @OpenApi(
      ...
      pathParams = {@OpenApiParam(name = "id", description = "Volunteer ID to delete")},
      responses = {
        @OpenApiResponse(status = "204", description = "Account deleted"),
        @OpenApiResponse(status = "400", description = "Cannot delete own account"),
        @OpenApiResponse(status = "500", description = "Database error")
      })
  ...
  @OpenApi(
      ...
      pathParams = {
        @OpenApiParam(name = "id", description = "The ID of the account whose password to change")
      },
      requestBody =
          @OpenApiRequestBody(
              required = true,
      ...
      responses = {
        @OpenApiResponse(status = "204", description = "Password updated"),
        @OpenApiResponse(status = "400", description = "Invalid request or wrong current password"),
        @OpenApiResponse(status = "403", description = "Cannot change another user's password"),
        @OpenApiResponse(status = "500", description = "Database error")
      })
  ...
  @OpenApi(
      ...
      pathParams = {@OpenApiParam(name = "id", description = "Volunteer ID")},
      requestBody =
          @OpenApiRequestBody(
              required = true,
              content = {@OpenApiContent(from = AddAffiliationRequest.class)}),
      responses = {
        @OpenApiResponse(status = "204", description = "Affiliation added"),
        @OpenApiResponse(status = "400", description = "Invalid role or missing chapter access"),
        @OpenApiResponse(status = "409", description = "Affiliation already exists"),
        @OpenApiResponse(status = "500", description = "Database error")
      })
  ...
  @OpenApi(
      ...
      pathParams = {
        @OpenApiParam(name = "id", description = "Volunteer ID"),
        @OpenApiParam(name = "chapterId", description = "Chapter ID")
      },
      requestBody =
          @OpenApiRequestBody(
              required = true,
              content = {@OpenApiContent(from = UpdateAffiliationRequest.class)}),
      responses = {
        @OpenApiResponse(status = "204", description = "Role updated"),
        @OpenApiResponse(status = "400", description = "Invalid role or missing chapter access"),
        @OpenApiResponse(status = "500", description = "Database error")
      })
  public static void updateAffiliation(Context ctx) {
    ...
    try {
      service.updateAffiliation(
          targetId, chapterId, request, requesterId, updaterRole, updaterChapterRoles);
      ctx.status(204);
    } catch (IllegalArgumentException e) {
      ctx.status(400).result(e.getMessage());
    } catch (SQLException e) {
  ...
  @OpenApi(
      ...
      pathParams = {
        @OpenApiParam(name = "id", description = "Volunteer ID"),
        @OpenApiParam(name = "chapterId", description = "Chapter ID to remove")
      },
      responses = {
        @OpenApiResponse(status = "204", description = "Affiliation removed"),
        @OpenApiResponse(status = "400", description = "Validation error"),
        @OpenApiResponse(status = "500", description = "Database error")
      })
