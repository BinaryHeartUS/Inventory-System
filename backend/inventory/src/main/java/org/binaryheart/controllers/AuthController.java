...
public class AuthController {
  ...
  /**
   * Throws 403 if the caller does not have a write-capable role (Editor, Chapter Admin, or Admin)
   * for the given chapter.
   *
   * <p>National membership is only sufficient if the caller's National role is itself write-capable
   * — a National Viewer cannot write to other chapters.
   */
  public static void requireChapterEditAccess(Context ctx, int chapterId) throws SQLException {
    ...
    for (ChapterRole cr : chapterRoles) {
      if (!WRITE_ROLES.contains(cr.role())) continue;
      if (cr.chapterId() == nationalId) return;
      if (cr.chapterId() == chapterId) return;
    }
  ...
  /**
   * Throws 403 if the caller is not affiliated with the given chapter in any role. Members of the
   * National chapter (any role) can read all chapters.
   */
