...
public class ChapterController {
  ...
  @OpenApi(
      ...
      responses = {
        @OpenApiResponse(
            ...
            content = {@OpenApiContent(from = ChapterSummary[].class)}),
  ...
  @OpenApi(
      ...
      description =
          "Creates a new chapter. Restricted to national admins: users with the Admin role "
              + "who are affiliated with the National chapter.",
      requestBody =
          @OpenApiRequestBody(
              required = true,
              content = {@OpenApiContent(from = CreateChapterRequest.class)}),
      responses = {
        @OpenApiResponse(
            status = "201",
            description = "Chapter created",
            content = {@OpenApiContent(from = ChapterSummary.class)}),
        @OpenApiResponse(status = "400", description = "Missing or blank chapter name"),
        @OpenApiResponse(
            status = "403",
  ...
  public static void deleteChapter(Context ctx) {
    ...
    try {
      int nationalId = service.getNationalChapterId();
      service.deleteChapter(
          id, nationalId, callerChapterIds != null ? callerChapterIds : List.of());
      ctx.status(204);
    } catch (IllegalArgumentException e) {
      ctx.status(400).result(e.getMessage());
    } catch (SQLException e) {
