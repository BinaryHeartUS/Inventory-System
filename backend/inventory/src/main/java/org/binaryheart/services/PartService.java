...
public class PartService {
  ...
  public PartResponse[] getAllParts(List<Integer> userChapterIds) throws SQLException {
    if (userChapterIds == null || userChapterIds.isEmpty()) return new PartResponse[0];
    if (userChapterIds.contains(chapterService.getNationalChapterId()))
      return repository.getAllParts();
    return List.of(repository.getAllParts()).stream()
        .filter(
            d -> {
  ...
  public PartResponse getPart(List<Integer> userChapterIds, Integer partId)
    ...
    if (userChapterIds == null || userChapterIds.isEmpty()) return null;

    PartResponse part = repository.getPart(partId);
    if ((part != null && userChapterIds.contains(part.chapterId()))
        || userChapterIds.contains(chapterService.getNationalChapterId())) return part;
  ...
  public PartResponse[] getPartsByDevice(List<Integer> userChapterIds, Integer deviceId)
    ...
    if (userChapterIds.contains(chapterService.getNationalChapterId())) return parts;
    return List.of(parts).stream()
        .filter(p -> userChapterIds.contains(p.chapterId()))
        .toArray(PartResponse[]::new);
  }
    ...
    try {
      ...
      if ("23505".equals(e.getSQLState())) {
        throw new DuplicateKeyException(
            "An asset with the same asset ID already exists: " + request.id());
      } else {
        throw e;
      }
    }
  ...
  public PartChangelogResponse[] getPartChangelog(List<Integer> userChapterIds, Integer partId)
    ...
    PartResponse part = repository.getPart(partId);
    if (part == null
        || (!userChapterIds.contains(part.chapterId())
            && !userChapterIds.contains(chapterService.getNationalChapterId()))) {
      throw new InvalidParameterException("Part not found");
    }
