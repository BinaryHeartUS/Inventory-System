...
public class LookupService {
  ...
  public LookupResponse getAll() throws SQLException {
    List<String> deviceStatuses =
        Arrays.stream(Status.values()).map(Status::getDatabaseValue).collect(Collectors.toList());
    List<String> chargerStatuses =
        Arrays.stream(ChargerStatus.values())
            .map(ChargerStatus::getDatabaseValue)
            .collect(Collectors.toList());
    List<String> workingBatteryOpts =
        Arrays.stream(WorkingBattery.values())
            .map(WorkingBattery::getDatabaseValue)
            .collect(Collectors.toList());
    return new LookupResponse(
        ...
        repository.getRamGenerations(),
        repository.getStorageTypes(),
        repository.getPartTypes(),
