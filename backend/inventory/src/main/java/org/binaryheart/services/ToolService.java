...
public class ToolService {
  ...
  public List<GetToolResponse> getAllTools(List<Integer> userChapterIds) throws SQLException {
    ...
    return repository.getAllTools().stream()
        .filter(
            d -> {
  ...
  public GetToolResponse getTool(List<Integer> userChapterIds, Integer toolID)
    ...
    if ((tool != null && userChapterIds.contains(tool.chapterId()))
        || userChapterIds.contains(chapterService.getNationalChapterId())) return tool;
    ...
    try {
      ...
      if ("23505".equals(e.getSQLState())) {
        throw new DuplicateKeyException(
            "An asset with identical ID already exists: " + request.assetId());
      } else {
        throw e;
      }
    }
    ...
    try {
      ...
      if ("02000".equals(e.getSQLState())) {
        throw new ToolNotFoundException(
            "Could not find tool with specified ID: " + request.assetId());
      } else {
        throw e;
      }
    }
  ...
  public ToolChangelogResponse[] getToolChangelog(List<Integer> userChapterIds, Integer toolId)
    ...
    GetToolResponse tool = repository.getTool(toolId);
    if (tool == null
        || (!userChapterIds.contains(tool.chapterId())
