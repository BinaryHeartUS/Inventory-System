...
public class AccountService {
  ...
  private static final Set<String> ADMIN_CREATABLE_ROLES =
      Set.of("Admin", "Chapter Admin", "Editor", "Viewer");
  private static final Set<String> CHAPTER_ADMIN_CREATABLE_ROLES = Set.of("Editor", "Viewer");
  private final ChapterService chapterService = new ChapterService();

  /** Throws if the role/chapter combination is invalid (e.g. Admin on a non-National chapter). */
    ...
    if ("Admin".equals(role)) {
      ...
      if (chapterId != nationalId) {
        throw new IllegalArgumentException(
            "The 'Admin' role can only be assigned to the National chapter");
      }
    }
  }
   ...
   * @param creatorRole the effective role of the volunteer performing the creation
   * @param creatorChapterRoles the per-chapter roles of the creator
   * @throws IllegalArgumentException if the role is not permitted or chapter access is missing
   * @throws DuplicateKeyException if the username is already taken
   */
  public AccountSummary createAccount(
      CreateAccountRequest request, String creatorRole, List<ChapterRole> creatorChapterRoles)
      throws SQLException {
    Set<String> allowed =
        "Admin".equals(creatorRole) ? ADMIN_CREATABLE_ROLES : CHAPTER_ADMIN_CREATABLE_ROLES;

    if (!allowed.contains(request.role())) {
      throw new IllegalArgumentException(
          "Role '" + request.role() + "' is not permitted for a " + creatorRole);
    }
    ...
    if (!"Admin".equals(creatorRole)) {
      ...
      // chapter
      boolean hasAccess =
          creatorChapterRoles.stream()
              .anyMatch(
                  cr -> cr.chapterId() == request.chapterId() && "Chapter Admin".equals(cr.role()));
      if (!hasAccess) {
        throw new IllegalArgumentException(
    ...
    try {
      int id =
          repository.createVolunteer(
              request.name(),
              request.username(),
              passwordHash,
              EncryptionHelper.getStringFromBytes(passwordSalt),
              request.chapterId(),
              request.role());
  ...
  /**
   * Returns accounts visible to the requesting volunteer. Admins see all; Chapter Admins see only
   * accounts in their chapters, excluding accounts whose effective role is higher than Editor.
   */
  public List<AccountSummary> getAccounts(String requesterRole, List<Integer> requesterChapterIds)
    ...
    return repository.getVolunteersForChapters(requesterChapterIds).stream()
        .filter(
            a ->
                a.chapterRoles().stream()
                    .allMatch(cr -> "Editor".equals(cr.role()) || "Viewer".equals(cr.role())))
        .toList();
  }

  /**
   * Deletes an account. Admins may delete any account except their own. Chapter Admins may delete
   * Editor/Viewer accounts in chapters they admin.
   */
  public void deleteAccount(
      int targetId, int requesterId, String requesterRole, List<ChapterRole> requesterChapterRoles)
      throws SQLException {
    ...
    if (!"Admin".equals(requesterRole)) {
      ...
      AccountSummary target =
          repository.getAllVolunteers().stream()
      ...
      for (ChapterRole cr : target.chapterRoles()) {
        if ("Chapter Admin".equals(cr.role()) || "Admin".equals(cr.role())) {
          throw new IllegalArgumentException(
              "You cannot delete accounts with Chapter Admin or Admin roles");
        }
        boolean adminsChapter =
            requesterChapterRoles.stream()
                .anyMatch(r -> r.chapterId() == cr.chapterId() && "Chapter Admin".equals(r.role()));
        if (!adminsChapter) {
          throw new IllegalArgumentException(
              "You can only delete accounts within your own chapters");
        }
      }
    }
  ...
  public void updatePassword(int volunteerId, String username, UpdatePasswordRequest request)
      throws SQLException {
    ...
    try {
      currentPasswordValid =
          EncryptionHelper.hashPassword(
                  EncryptionHelper.DECODER.decode(credentials.passwordSalt()),
    ...
    repository.updatePassword(
        volunteerId, passwordHash, EncryptionHelper.getStringFromBytes(passwordSalt));
  }

  /**
   * Updates the role for an existing chapter affiliation. Applies the same chapter/role permission
   * checks as addAffiliation.
   */
  public void updateAffiliation(
      int targetId,
      int chapterId,
      UpdateAffiliationRequest request,
      int requesterId,
      String updaterRole,
      List<ChapterRole> updaterChapterRoles)
      throws SQLException {
    if (targetId == requesterId) {
    ...
    Set<String> allowed =
        "Admin".equals(updaterRole) ? ADMIN_CREATABLE_ROLES : CHAPTER_ADMIN_CREATABLE_ROLES;

    if (!allowed.contains(request.role())) {
      throw new IllegalArgumentException(
          "Role '" + request.role() + "' is not permitted for a " + updaterRole);
    }
  ...
  /**
   * Removes a chapter affiliation from an account. Admins may remove any affiliation. Chapter
   * Admins may only remove Editor/Viewer affiliations from chapters they admin. The last
   * affiliation on an account cannot be removed (the account must be deleted instead).
   */
  public void removeAffiliation(
      ...
      int requesterId,
      String removerRole,
      List<ChapterRole> removerChapterRoles)
      throws SQLException {
    if (targetId == requesterId) {
    ...
    ChapterRole affiliation =
        target.chapterRoles().stream()
            .filter(cr -> cr.chapterId() == chapterId)
            .findFirst()
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "This account has no affiliation with that chapter"));
  ...
  /**
   * Adds a chapter+role affiliation to an existing account. Applies the same role/chapter
   * permission checks as createAccount.
   */
  public void addAffiliation(
      int targetId,
      AddAffiliationRequest request,
      String creatorRole,
      List<ChapterRole> creatorChapterRoles)
      throws SQLException {
    Set<String> allowed =
        "Admin".equals(creatorRole) ? ADMIN_CREATABLE_ROLES : CHAPTER_ADMIN_CREATABLE_ROLES;
